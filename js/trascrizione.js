...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function readAndResampleAudio(file) {
    ...
    try {
      ...
      throw new Error('formato-audio');
    }
    // Un'ora di audio occupa circa 230 MB in memoria: meglio avvisare prima di iniziare
    if (audioBuffer.duration > 3600) {
      await audioCtx.close();
      throw new Error('audio-troppo-lungo');
    }
  ...
  btnTranscribe?.addEventListener('click', async () => {
    ...
    try {
      ...
      console.error(error);
      const causa = String(error && error.message);
      let messaggio = 'Errore durante la trascrizione.';
      if (causa === 'formato-audio') messaggio = 'Formato audio non riconosciuto dal browser: converti il file in MP3, WAV o M4A e riprova.';
      else if (causa === 'audio-troppo-lungo') messaggio = 'Registrazione superiore a un\'ora: dividila in parti più brevi prima di trascriverla.';
      else if (/fetch|network|load|Failed/i.test(causa)) messaggio = 'Impossibile scaricare il modello di riconoscimento: controlla la connessione e riprova.';
      modelStatus.textContent = messaggio;
      resultArea.value = '';
      resetUI();
    }
  });
  ...
  btnDownloadSrt?.addEventListener('click', () => {
    ...
    const base = (audioFile?.name || 'trascrizione').replace(/\.[^.]+$/, '');
    scarica(srtContent, `${base}.srt`, 'application/x-subrip;charset=utf-8');
  });

  document.getElementById('btn-download-txt')?.addEventListener('click', () => {
    if (!resultArea.value) return;
    const base = (audioFile?.name || 'trascrizione').replace(/\.[^.]+$/, '');
    scarica(resultArea.value, `${base}.txt`, 'text/plain;charset=utf-8');
  });
});
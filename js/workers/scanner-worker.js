...
const scannerService = {
  ...
  async detectCorners(imageData) {
    ...
    try {
      ...
      // Il contorno del foglio si chiude solo se i bordi sono netti: si tenta prima con le soglie
      // standard e, se non emerge alcun quadrilatero, con soglie più basse per i fogli chiari su
      // sfondo chiaro. Soglie più basse non bastano da sole: introdurrebbero bordi spuri nelle foto nitide.
      const imageArea = width * height;
      let bestCandidate = null;

      const puntiDaMat = (mat) => {
        const punti = [];
        for (let p = 0; p < 4; p++) punti.push({ x: mat.data32S[p * 2] / scale, y: mat.data32S[p * 2 + 1] / scale });
        return punti;
      };

      for (const [sogliaBassa, sogliaAlta] of [[50, 150], [20, 60]]) {
        if (edges) edges.delete();
        edges = new cv.Mat();
        cv.Canny(blurred, edges, sogliaBassa, sogliaAlta, 3, false);
        // I bordi rilevati da Canny sono spesso interrotti da ombre e riflessi: una chiusura
        // morfologica ricongiunge i segmenti vicini, così il contorno del foglio risulta chiuso.
        const nucleo = cv.getStructuringElement(cv.MORPH_RECT, new cv.Size(5, 5));
        cv.morphologyEx(edges, edges, cv.MORPH_CLOSE, nucleo);
        nucleo.delete();
        ...
        for (let i = 0; i < contours.size(); i++) {
          ...
          if (areaRatio >= CONFIG.minAreaRatio && areaRatio <= CONFIG.maxAreaRatio) {
            // L'inviluppo convesso elimina le rientranze dei bordi frastagliati; l'approssimazione
            // poligonale viene tentata con tolleranze crescenti prima di scartare il candidato.
            const hull = new cv.Mat();
            cv.convexHull(contour, hull, false, true);
            const perimeter = cv.arcLength(hull, true);
            for (const tolleranza of [0.02, 0.04, 0.06]) {
              const approx = new cv.Mat();
              cv.approxPolyDP(hull, approx, perimeter * tolleranza, true);
              const quadrilatero = approx.rows === 4;
              if (quadrilatero && cv.isContourConvex(approx) && area > maxArea) {
                maxArea = area;
                bestCandidate = orderPoints(puntiDaMat(approx));
              }
              approx.delete();
              if (quadrilatero) break;
            }
            hull.delete();
          }
          contour.delete();

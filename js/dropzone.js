...
(function () {
  ...
  // L'attributo capture apre direttamente la fotocamera e impedisce di scegliere un file già
  // presente sul telefono. L'input principale resta quindi senza capture (il sistema mostra sia la
  // fotocamera sia la galleria) e lo scatto diretto viene offerto da un secondo input dedicato.
  function collegaFotocamera(options) {
    const opts = options || {};
    const principale = resolveEl(opts.input);
    const fotocamera = resolveEl(opts.camera);
    const bottone = resolveEl(opts.button);
    if (!principale || !fotocamera) return null;

    if (bottone) bottone.addEventListener('click', (e) => { e.preventDefault(); e.stopPropagation(); fotocamera.click(); });

    fotocamera.addEventListener('change', () => {
      const files = getInputFiles(fotocamera);
      if (files && files.length) setFilesOnInput(principale, files);
      fotocamera.value = '';
    });

    return { apri: () => fotocamera.click() };
  }

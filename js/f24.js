(() => {
  ...
  function initF24App() {
    ...
    // ==========================================
    // 3. GENERAZIONE DINAMICA DELLE RIGHE & PERSISTENZA
    // ==========================================
    const rowTemplates = {
    ...
    function addRow(targetId, values = []) {
      const container = document.getElementById(targetId);
      if (!container) return;

      if (targetId === 'container-f23-righe' && container.children.length >= 4) {
        if (!values.length) alert('Il modello F23 ammette un massimo di 4 righe tributo.');
        return;
      }
    ...
    function saveF24State() {
      if (!window.AppStorage) return;
      const state = { activeModel, containers: {} };

      Object.keys(rowTemplates).forEach(containerId => {
        const container = document.getElementById(containerId);
        if (!container) return;
        state.containers[containerId] = [];

        const rows = container.querySelectorAll('.f24-row');
        rows.forEach(r => {
          const rowValues = Array.from(r.querySelectorAll('input, select')).map(i => i.value);
          state.containers[containerId].push(rowValues);
        });
      });

      window.AppStorage.save('f24_rows_state', state);
    }

    function loadF24State() {
      if (!window.AppStorage) return;
      const state = window.AppStorage.load('f24_rows_state', null);
      ...
      Object.keys(state.containers).forEach(containerId => {
        ...
        if (state.containers[containerId] && state.containers[containerId].length > 0) {
          container.innerHTML = '';
          state.containers[containerId].forEach(valArray => {
            addRow(containerId, valArray);
          });
        }
      });
    ...
    function recalculateTotals() {
      ...
      if (saldoWarning) {
        ...
        // Il limite di 1.000 € per il cartaceo è stato abolito dal D.L. 193/2016: conta la presenza di compensazioni
        else if (totCrediti > 0 && activeModel !== 'f23') {
          saldoWarning.innerHTML = '⚡ <strong>Obbligo Telematico:</strong> I modelli con crediti compensati vanno presentati tramite i servizi telematici dell\'Agenzia delle Entrate, non allo sportello né con l\'home banking.';
          saldoWarning.classList.remove('hidden', 'text-rose-500');
          saldoWarning.classList.add('text-amber-500');
        }
      }
    ...
    if (btnGeneratePdf) {
      btnGeneratePdf.addEventListener('click', async () => {
        if (typeof PDFLib === 'undefined' || typeof window.fontkit === 'undefined') {
          alert("Caricamento librerie in corso, riprova tra un istante.");
          return;
        }
        ...
        try {
          ...
          // 5.0 REGISTRAZIONE FONTKIT E CARICAMENTO FONT TTF REALES
          pdfDoc.registerFontkit(window.fontkit);

          // Scarichiamo le fonti TTF (puoi anche salvarle localmente sul tuo server in futuro)
          ...
          // Incrustamos los bytes de la fuente directamente en el PDF
          const fontCourier = await pdfDoc.embedFont(fontBytesReg);
          const fontCourierBold = await pdfDoc.embedFont(fontBytesBold);
          const fontHelveticaBold = await pdfDoc.embedFont(fontBytesBold);
          ...
          // 5.5 INIEZIONE METADATI ISO PDF/A E PROFILO ICC
...
<?xpacket end="w"?>`;
          ...
          // AGGIUNTA DEL PROFILO COLORE ICC
          ...
          // SALVATAGGIO (Senza Object Streams come richiede PDF/A-1)
          ...
          console.error(err);
          alert('Errore durante la creazione del PDF. Assicurati che i campi siano compilati correttamente e di avere una connessione internet per scaricare i font.');
        } finally {

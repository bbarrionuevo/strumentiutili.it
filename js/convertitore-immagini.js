(function(){
  ...
  function setStatus(t){ if (progress) { progress.textContent = t; progress.classList.remove('hidden'); } }
    ...
    return new Promise((resolve, reject)=>{
      ...
      img.onload = async ()=>{
        try{
          ...
          // JPEG non ha canale alfa: senza sfondo i pixel trasparenti diventerebbero neri
          if (targetType === 'image/jpeg') { ctx.fillStyle = '#ffffff'; ctx.fillRect(0, 0, canvas.width, canvas.height); }
          ...
          canvas.toBlob((blob)=>{
            ...
            // I browser che non sanno codificare il formato richiesto (spesso AVIF) restituiscono un PNG senza errori
            if (blob.type !== targetType) return reject(new Error('Questo browser non supporta la creazione di file ' + (targetType.split('/')[1] || '').toUpperCase() + ': scegli WebP, PNG o JPEG'));
            resolve(blob);
          }, targetType, q);

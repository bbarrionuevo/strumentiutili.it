  ...
  if (generateBtn) {
    generateBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        const nonCodificabile = /WinAnsi cannot encode "(.+?)"/.exec(String(err && err.message));
        const message = nonCodificabile
          ? `Il carattere «${nonCodificabile[1]}» non è supportato dal font del PDF. Sostituiscilo con la lettera senza segni (es. «S» invece di «Ș») e, se serve, correggilo a penna sul documento stampato.`

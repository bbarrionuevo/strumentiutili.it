...
(() => {
  ...
  // Fallbacks de seguridad en caso de fallo del JSON
  const DEFAULT_PRESTAZIONE = {
    ritenutaAcconto: 0.20,
    sogliaInps: 5000.00
  };

  const DEFAULT_BOLLO = {
    sogliaEsenzione: 77.47,
    importo: 2.00
  };
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Shielding contra undefined
    const config = (regole && regole.prestazioneOccasionale) 
      ? regole.prestazioneOccasionale 
      : DEFAULT_PRESTAZIONE;

    const configBollo = (regole && regole.generale && regole.generale.marcaDaBollo) 
      ? regole.generale.marcaDaBollo 
      : DEFAULT_BOLLO;
    ...
    function calculate() {
      ...
      if (mode === 'lordo_to_netto') {
        ...
        lordo = rawVal;
        ritenuta = lordo * config.ritenutaAcconto;
        netto = (lordo - ritenuta) + rimborsi;
      } else {
        labelAmount.textContent = 'Netto Desiderato (€)';
        const nettoSenzaRimborsi = Math.max(0, rawVal - rimborsi);
        const fattoreScorporo = 1 / (1 - config.ritenutaAcconto);
        lordo = nettoSenzaRimborsi * fattoreScorporo;
        ritenuta = lordo * config.ritenutaAcconto;
        netto = rawVal;
      }

      const bolloDovuto = lordo > configBollo.sogliaEsenzione;

      currentCalculations = { lordo, ritenuta, rimborsi, netto, bolloDovuto };

      if (outLordo) outLordo.textContent = formatEuro(lordo);
      if (outRitenuta) outRitenuta.textContent = `- ${formatEuro(ritenuta)}`;
      if (outRimborsi) outRimborsi.textContent = formatEuro(rimborsi);
      if (outNetto) outNetto.textContent = formatEuro(netto);
    ...
    // Event Listeners
    if (calcMode) calcMode.addEventListener('change', calculate);
    if (inputAmount) inputAmount.addEventListener('input', calculate);
    if (inputRimborsi) inputRimborsi.addEventListener('input', calculate);
    ...
    btnGeneratePdf.addEventListener('click', async () => {
      ...
      try {
        ...
        currY -= 22;
        const percentualeRitenuta = (config.ritenutaAcconto * 100).toFixed(0);
        drawRow(`Ritenuta d'Acconto ${percentualeRitenuta}% (A carico del Committente)`, `- ${formatEuro(currentCalculations.ritenuta)}`, currY);
        currY -= 22;
        ...
        if (currentCalculations.bolloDovuto) {
          ...
          const importoBolloFmt = formatEuro(configBollo.importo);
          const bolloText = bolloID 
            ? `Imposta di bollo di ${importoBolloFmt} assolta sull'originale (ID Marca: ${bolloID}).`
            : `Imposta di bollo di ${importoBolloFmt} assolta sull'originale con identificativo da applicare.`;
